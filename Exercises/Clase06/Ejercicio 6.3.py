...
def propagar(l, debug = True):
    ...
    if debug:
        print(s)#, end = ' -> ')
    #OJO QUE EL SPLIT SACA lAS 'X' al hacer su trabajo
    W=s.split('x') 
    PW=[w if ('f' not in w) else 'f'*len(w) for w in W]
    # Como el split sacó las 'x' con el join() hay que agregarlas
    # ps=''.join(PW) # codigo no funcional, hay que arreglarlo
    ps='x'.join(PW)
    if debug:
        print(ps)
    return trad2l(ps)
