...
def f_principal():
    ...
    plt.subplot(2, 1, 1) 
    for caminata in caminatas:
        if sum(caminata) == sum(max(distancias)[1]): #sumo todos los elementos para comparar las matrices
            plt.plot(caminata, color='red')
        elif sum(caminata) == sum(min(distancias)[1]): #sumo todos los elementos para comparar las matrices
            plt.plot(caminata, color='black')
        else:
