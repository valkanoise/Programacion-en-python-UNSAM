'''
    Es el script random_walk.py pero en este caso se observan en el subplot
    ppal las caminatas que más y menos se alejan del origen.
    En esta versión plus2 hay menos gasto computacional porque no sumo
    todos los elementos de las matrices en el subplot principal.
    En este caso se grafican 14 series, las 12 caminatas, y por encima las
    caminatas que MAS y MENOS se alejan del origen.
'''
...
def f_principal():
    ...
    ###### Modifico para que marqué las distancias min y max
    plt.subplot(2, 1, 1) 
    # Grafico todas las 12 series
    for caminata in caminatas:
        plt.plot(caminata, color='blue', alpha = 0.5)
    # Adicionalmente vuelvo a graficar en otro color...
    # Grafico la que más se aleja del origen
    plt.plot(max(distancias)[1], color = 'red')
    # Grafico la que menos se aleja del origen
    plt.plot(min(distancias)[1], color = 'black')
